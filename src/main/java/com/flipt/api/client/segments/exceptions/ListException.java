package com.flipt.api.client.segments.exceptions;
...
@JsonDeserialize(
    using = ListException.Deserializer.class
)
public final class ListException extends Exception {
  private final Value value;

  private int statusCode;

  private ListException(Value value, int statusCode) {
    this.value = value;
  ...
  public static ListException other(Object unknownValue, int statusCode) {
    return new ListException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "ListException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<ListException> {
    @Override
    public ListException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new ListException(value, statusCode);
    }
  }
}
