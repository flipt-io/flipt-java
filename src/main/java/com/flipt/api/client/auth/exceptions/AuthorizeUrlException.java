...
@JsonDeserialize(
    using = AuthorizeUrlException.Deserializer.class
)
public final class AuthorizeUrlException extends Exception {
  private final Value value;

  private int statusCode;

  private AuthorizeUrlException(Value value, int statusCode) {
    this.value = value;
  ...
  public static AuthorizeUrlException other(Object unknownValue, int statusCode) {
    return new AuthorizeUrlException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "AuthorizeUrlException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<AuthorizeUrlException> {
    @Override
    public AuthorizeUrlException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new AuthorizeUrlException(value, statusCode);
    }
  }
}
