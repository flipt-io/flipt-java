...
@JsonDeserialize(
    using = CreateTokenException.Deserializer.class
)
public final class CreateTokenException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateTokenException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateTokenException other(Object unknownValue, int statusCode) {
    return new CreateTokenException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateTokenException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateTokenException> {
    @Override
    public CreateTokenException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateTokenException(value, statusCode);
    }
  }
}
