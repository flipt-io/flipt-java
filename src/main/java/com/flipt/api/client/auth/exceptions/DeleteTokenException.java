...
@JsonDeserialize(
    using = DeleteTokenException.Deserializer.class
)
public final class DeleteTokenException extends Exception {
  private final Value value;

  private int statusCode;

  private DeleteTokenException(Value value, int statusCode) {
    this.value = value;
  ...
  public static DeleteTokenException other(Object unknownValue, int statusCode) {
    return new DeleteTokenException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "DeleteTokenException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<DeleteTokenException> {
    @Override
    public DeleteTokenException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new DeleteTokenException(value, statusCode);
    }
  }
}
