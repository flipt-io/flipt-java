...
@JsonDeserialize(
    using = GetSelfException.Deserializer.class
)
public final class GetSelfException extends Exception {
  private final Value value;

  private int statusCode;

  private GetSelfException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetSelfException other(Object unknownValue, int statusCode) {
    return new GetSelfException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetSelfException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetSelfException> {
    @Override
    public GetSelfException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetSelfException(value, statusCode);
    }
  }
}
