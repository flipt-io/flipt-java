...
@JsonDeserialize(
    using = CallbackException.Deserializer.class
)
public final class CallbackException extends Exception {
  private final Value value;

  private int statusCode;

  private CallbackException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CallbackException other(Object unknownValue, int statusCode) {
    return new CallbackException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CallbackException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CallbackException> {
    @Override
    public CallbackException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CallbackException(value, statusCode);
    }
  }
}
