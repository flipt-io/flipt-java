...
@JsonDeserialize(
    using = ExpireSelfException.Deserializer.class
)
public final class ExpireSelfException extends Exception {
  private final Value value;

  private int statusCode;

  private ExpireSelfException(Value value, int statusCode) {
    this.value = value;
  ...
  public static ExpireSelfException other(Object unknownValue, int statusCode) {
    return new ExpireSelfException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "ExpireSelfException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<ExpireSelfException> {
    @Override
    public ExpireSelfException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new ExpireSelfException(value, statusCode);
    }
  }
}
