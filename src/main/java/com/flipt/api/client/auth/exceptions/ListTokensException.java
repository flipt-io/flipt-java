...
@JsonDeserialize(
    using = ListTokensException.Deserializer.class
)
public final class ListTokensException extends Exception {
  private final Value value;

  private int statusCode;

  private ListTokensException(Value value, int statusCode) {
    this.value = value;
  ...
  public static ListTokensException other(Object unknownValue, int statusCode) {
    return new ListTokensException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "ListTokensException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<ListTokensException> {
    @Override
    public ListTokensException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new ListTokensException(value, statusCode);
    }
  }
}
