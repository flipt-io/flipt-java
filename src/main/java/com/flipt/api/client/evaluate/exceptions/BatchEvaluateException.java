package com.flipt.api.client.evaluate.exceptions;
...
@JsonDeserialize(
    using = BatchEvaluateException.Deserializer.class
)
public final class BatchEvaluateException extends Exception {
  private final Value value;

  private int statusCode;

  private BatchEvaluateException(Value value, int statusCode) {
    this.value = value;
  ...
  public static BatchEvaluateException other(Object unknownValue, int statusCode) {
    return new BatchEvaluateException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "BatchEvaluateException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<BatchEvaluateException> {
    @Override
    public BatchEvaluateException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new BatchEvaluateException(value, statusCode);
    }
  }
}
